...
ARG HOST_VERSION=4.0.0-preview.X.00000
FROM mcr.microsoft.com/dotnet/sdk:6.0-bullseye-slim AS runtime-image
ARG HOST_VERSION

# Build requires 3.1 SDK
COPY --from=mcr.microsoft.com/dotnet/sdk:5.0-bullseye-slim /usr/share/dotnet /usr/share/dotnet
...
RUN BUILD_NUMBER=$(echo ${HOST_VERSION} | cut -d'.' -f 5) && \
    ...
    dotnet restore --disable-parallel src/WebJobs.Script.WebHost/WebJobs.Script.WebHost.csproj && \
    dotnet publish -v q /p:VersionSuffix=$SUFFIX /p:BuildNumber=$BUILD_NUMBER /p:CommitHash=$HOST_COMMIT src/WebJobs.Script.WebHost/WebJobs.Script.WebHost.csproj -c Release --no-restore --output /azure-functions-host && \
...
FROM mcr.microsoft.com/dotnet/nightly/aspnet:6.0-bullseye-slim
ARG HOST_VERSION
